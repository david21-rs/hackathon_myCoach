...
public class IntroManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI speakerText;   // shows "Professor Doofenshmirtz"
    public TextMeshProUGUI dialogueText;  // shows the actual speech
    public Image background;             // the background image
    public Sprite labBackground;         // the lab scene image
    public Sprite mapBackground;         // the map scene image

    // All the dialogue lines in order
    private string[] lines = new string[]
    {
        ...
        "SWITCH_TO_MAP",   // ← this triggers the background change
    ...
    void Update()
    {
        // Player presses Space or Enter to go to next line
    ...
    void ShowCurrentLine()
    {
        ...
        // Check if this is a background switch command
        if (line == "SWITCH_TO_MAP")
        {
            background.sprite = mapBackground;
            currentLine++;
            ShowCurrentLine();   // immediately show next line
            return;
        }
        ...
        // Otherwise just show the dialogue
    ...
    void NextLine()
    {
        ...
        // All lines done — load the title card then the game
        if (currentLine >= lines.Length)
        {
            SceneManager.LoadScene("TitleScene"); // or "SavannaScene" to skip title
            return;
        }
